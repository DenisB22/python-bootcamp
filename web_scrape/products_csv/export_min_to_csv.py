...
def export_only_cols_to_csv_min() -> bool:
    try:
        with open('./products_csv/products_min.csv', mode='r') as csvfile:
...
def export_only_rows_to_csv_min(product: dict) -> None:
    ...
    df.to_csv('./products_csv/products_min.csv', mode='a', index=False, header=False)
...
def export_rows_and_cols_to_csv_min(product: dict) -> None:
    ...
    df.to_csv("./products_csv/products_min.csv", index=False, columns=fields)
